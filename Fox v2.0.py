...
for i in range(count):
    ...
    current_time = datetime.now().strftime("%H:%M:%S:").replace(":", "s").replace("s", "m", 2).replace("m", "h", 1)
    
    os.system(f"curl {img_url} --output img_{i + 1}[on_{current_date}_at_{current_time}].jpg")
